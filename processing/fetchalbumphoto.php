	...
	if (isset($photoId) && !empty($photoId))
	{
		...
		if ($dbConn->connect_errno == 0 && $dbConn->select_db(IWANSHOKOTO_DBNAME))
		{
			...
			if (isset($decode) && $decode == "true")
				$photoId = decodeKey($photoId);
				
			$sql = sprintf("SELECT Photo, PhotoFormat AS ImageFormat FROM album_photo WHERE PhotoGenID='%s' AND AlbumGenID='%s'", $photoId, $albumId);
			...
			if ($photoResult && $photoResult->num_rows > 0)
			{
				...
				if (!empty($photoRow['ImageFormat']))
				{
					...
					if ($imgOptimize->setImage($imageHnd))
					{	
					...
					{
						//send default
						$defaultImageHnd = imagecreatefromstring(file_get_contents("../images/defaultavatar.png"));
						header('Content-type: image/png');
						imagepng($defaultImageHnd);
					}
				}
			}
		}
