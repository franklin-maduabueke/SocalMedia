            	...
            	<div id="rightcolcontentholder">
            		<div id="rightcolheading">
                		<div id="rightcoldescription">
                        	<label style="color:#333333;">Match Results </label>
                            <label style="color:#E62174">(<?php if (array_key_exists($_SESSION[SITEUSERAUTHENTICATIONKEY], $matchCollection)){ $totalSearchResult = count($matchCollection) - 1; echo number_format($totalSearchResult);} else echo number_format(count($matchCollection));
							?>)</label><!--<label style="color:#007DB8"><?php echo $_SESSION['TheUser'];?></label>--></div>
                    	...
                    	<?php
							if (isset($matchCollection) && count($matchCollection) > 0)
							{
								foreach ($matchCollection as $key=>$value)
								{
									...
									{
										if ($json->memgenid == $_SESSION[SITEUSERAUTHENTICATIONKEY])
											continue;
						?>
